...
`profile="kicad_cli"` remains metadata-free.
...
- drill geometry exposes `data-primitive="pad-hole"` or
  `data-primitive="via-hole"` with owner/type/plating metadata
...
4. Embed PCB document metadata payload and root attributes for review/enriched
   output only.
5. Add focused L0 conformance tests for:
   - `profile="kicad_cli"` suppressing all enrichment
